 ...
 * Copyright 2020 - Katherina Navarrete, karen flores
 *
 */
                            ...
                            unsigned char color)
{
	...
	int i = x; /* para recorrer el font por columnas */
	int j = y; /* para recorrer el font por filas */
	char *col = &font[c*5]; /* para almacenar cada columna de pixeles del font */
	
	int tmp = 1; 
      	for(int m=0;m < 5; m++){
		for(int n=0;n <8; n++){
			if((*col & tmp) == tmp){
				draw_pixel_on_buffer(i, j, 1);
			}
			else{
				draw_pixel_on_buffer(i, j, 0);}
			tmp =tmp*2;
			j++;
		}
		col +=1;
